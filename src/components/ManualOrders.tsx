...
export function ManualOrders({ orders, onChanged }: Props) {
  ...
  const open = (order: OrderRecord) => {
    setSelected(order)
    setStatus('filled')
    setFillPrice(order.price)
    setFillQuantity(order.quantity)
    setBrokerOrderId('')
    setError('')
  }
  ...
  const save = async () => {
    ...
    setError('')
    try {
      await api.reconcileManualOrder(selected.id, { status, fillPrice, fillQuantity, brokerOrderId })
      setSelected(null)
      await onChanged()
    } catch (reason) {
      ...
      <div className="manual-orders-list">
        {orders.map((order) => (
          <div className="manual-order-row" key={order.id}>
            ...
            <span><strong>{new Date(order.submittedAt).toLocaleDateString('zh-CN')}</strong><small>{new Date(order.submittedAt).toLocaleTimeString('zh-CN', { hour12: false })}</small></span>
            {order.status === 'awaiting_broker_confirmation' ? (
              <button className="button secondary small" type="button" onClick={() => open(order)}>回填结果</button>
        ...
        <div className="dialog-backdrop" role="presentation">
          <div className="dialog small-dialog" role="dialog" aria-modal="true" aria-labelledby="reconcile-title">
            ...
            <div className="segmented status-segmented">
              <button type="button" className={status === 'filled' ? 'active' : ''} onClick={() => setStatus('filled')}>已成交</button>
              <button type="button" className={status === 'partially_filled' ? 'active' : ''} onClick={() => setStatus('partially_filled')}>部分成交</button>
              <button type="button" className={status === 'cancelled' ? 'active' : ''} onClick={() => setStatus('cancelled')}>已撤单</button>
              <button type="button" className={status === 'rejected' ? 'active' : ''} onClick={() => setStatus('rejected')}>已拒绝</button>
            </div>
            {['filled', 'partially_filled'].includes(status) && (
